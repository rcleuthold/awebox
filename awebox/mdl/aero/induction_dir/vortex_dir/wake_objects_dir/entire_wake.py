...
import awebox.mdl.aero.induction_dir.vortex_dir.wake_objects_dir.bound_wake as wod_bound_wake
import awebox.mdl.aero.induction_dir.vortex_dir.wake_objects_dir.near_wake as wod_near_wake
import awebox.mdl.aero.induction_dir.vortex_dir.wake_objects_dir.far_wake as wod_far_wake
...
def build(options, architecture, wind, variables_si, variables_scaled, parameters):
    ...
    bound_wake = wod_bound_wake.build(options, architecture, variables_si, parameters)
    near_wake = wod_near_wake.build(options, architecture, variables_si, parameters)
    far_wake = wod_far_wake.build(options, architecture, wind, variables_si, parameters)
...
def test_drawing(test_includes_visualization):
    ...
    if test_includes_visualization:
        ...
        wake = build(options, architecture, wind, variables_si, variables_si, parameters)
...
def test(test_includes_visualization=False):
    wod_bound_wake.test(test_includes_visualization)
    wod_near_wake.test(test_includes_visualization)
    wod_far_wake.test(test_includes_visualization)
