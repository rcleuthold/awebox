...
def try_making_node_position_constraint(nlp_options, V, P, Xdot, Outputs, model, time_grids):
    ...
    for kite_shed in model.architecture.kite_nodes:
        parent_shed = model.architecture.parent_map[kite_shed]
        q_kite_name = 'q' + str(kite_shed) + str(parent_shed)

        for tip in ['int', 'ext']:
            for wake_node in range(1, wake_nodes):
                diff_name = vortex_tools.get_wake_node_position_name(kite_shed=kite_shed, tip=tip,
                                                                       wake_node=wake_node)

                for ndx in range(n_k):
                    for ddx in [None]:

                        q_kite_scaled = V['x', ndx, q_kite_name]
                        diff_scaled = V['z', ndx, diff_name]

                        q_kite_si = struct_op.var_scaled_to_si('x', q_kite_name, q_kite_scaled, model.scaling)
                        diff_si = struct_op.var_scaled_to_si('z', diff_name, diff_scaled, model.scaling)
                        fixing_position_si = vortex_tools.combine_diff_and_qkite_into_wake_node_position_si(q_kite_si, diff_si)

                        convected_position_si = try_getting_convected_position_si(nlp_options, kite_shed, tip, wake_node, tgrid_coll, V, P, Xdot, model, ndx, ddx)
                        resi_si = fixing_position_si - convected_position_si
                        resi_scaled = struct_op.var_si_to_scaled('z', diff_name, resi_si, model.scaling)
                        resi_expr = cas.vertcat(resi_expr, resi_scaled)

                    for ddx in range(d):

                        q_kite_scaled = V['coll_var', ndx, ddx, 'x', q_kite_name]
                        diff_scaled = V['coll_var', ndx, ddx, 'z', diff_name]
...
def try_getting_convected_position_si(nlp_options, kite_shed, tip, wake_node, tgrid_coll, V, P, Xdot, model, ndx, ddx):
    ...
    anchor_ndx = ndx - 1
    anchor_node = wake_node - 1

    anchoring_variables_scaled = struct_op.get_variables_at_time(nlp_options, V, Xdot, model.variables, anchor_ndx, ddx)
    anchoring_variables_si = struct_op.variables_scaled_to_si(model.variables, anchoring_variables_scaled, model.scaling)
    anchoring_position_si = vortex_tools.get_wake_node_position_si(model.options, anchoring_variables_si, kite_shed, tip, anchor_node, model.architecture)
    ...
    if nlp_options['induction']['vortex_convection_type'] == 'free':
        ...
        fixing_variables_scaled = struct_op.get_variables_at_time(nlp_options, V, Xdot, model.variables, ndx, ddx)
        fixing_variables_si = struct_op.variables_scaled_to_si(model.variables, fixing_variables_scaled, model.scaling)
        fixing_position_si = vortex_tools.get_wake_node_position_si(model.options, fixing_variables_si, kite_shed, tip,
                                                                       wake_node, model.architecture)

        x_halfways = (fixing_position_si + anchoring_position_si) / 2.
        parameters = struct_op.get_parameters_at_time(V, P, model.parameters)
        u_ind = model.wake.calculate_total_biot_savart_at_x_obs(fixing_variables_scaled, parameters, x_obs=x_halfways)
        u_local += u_ind

    convection_distance_si = convection_time * u_local
    return anchoring_position_si + convection_distance_si
...
def get_specific_local_constraint(abbreviated_var_name, nlp_options, V, Outputs, Integral_outputs, model, time_grids, kite_shed_or_parent_shed, tip,
    ...
    if ddx is None:
        ...
        if abbreviated_var_name == 'wx':
            var_value_si = get_local_convected_position_diff_value(nlp_options, V, Outputs, model, time_grids, kite_shed_or_parent_shed, tip, wake_node_or_ring, ndx, ddx)
...
def get_local_convected_position_diff_value(nlp_options, V_scaled, Outputs, model, time_grids, kite_shed, tip, wake_node, ndx, ddx):
    t_f_scaled = V_scaled['theta', 't_f']
    t_f_si = struct_op.var_scaled_to_si('theta', 't_f', t_f_scaled, model.scaling)
    tgrid_coll = time_grids['coll'](t_f_si)
    ...
    parent = model.architecture.parent_map[kite_shed]
    q_kite_name = 'q' + str(kite_shed) + str(parent)
    q_kite_scaled = V_scaled['coll_var', ndx, ddx, 'x', q_kite_name]
    q_kite_si = struct_op.var_scaled_to_si('x', q_kite_name, q_kite_scaled, model.scaling)
    diff_si = vortex_tools.separate_position_and_qkite_into_wake_node_diff_si(q_kite_si, wx_convected)
...
def get_the_convected_position_from_the_current_indices_and_wake_node(nlp_options, Outputs, model, tgrid_coll, kite, tip, wake_node, ndx, ddx=None):
    ...
    if nlp_options['induction']['vortex_convection_type'] == 'free':
        message = 'diff_repr vortex model not yet set up for free vortex convection. proceeding with rigid convection'
