...
def check_that_wake_node_0_always_has_a_convection_time_of_zero(time_grids, V_init_scaled, init_options, n_k, d):
    ...
    for ndx in range(n_k):
        conv_time_control = diff_fixing.get_the_convection_time_from_the_current_indices_and_wake_node(init_options,
                                                                                                          tcoll,
                                                                                                          wake_node=0,
                                                                                                          ndx=ndx)
    ...
    for ddx in range(d):
            conv_time_collocation = diff_fixing.get_the_convection_time_from_the_current_indices_and_wake_node(init_options, tcoll, wake_node=0, ndx=ndx, ddx=ddx)
    ...
    conv_time_final = diff_fixing.get_the_convection_time_from_the_current_indices_and_wake_node(init_options,
                                                                                                  tcoll,
                                                                                                  wake_node=0,
                                                                                                  ndx=-1)
...
def check_that_wake_node_0_always_lays_on_wingtips(init_options, V_init_si, Outputs_init, architecture):
    ...
    for kite in architecture.kite_nodes:
        ...
        q_kite_name = 'q' + str(kite) + str(parent)
        ...
        for tip in ['int', 'ext']:
            ...
            for ndx in range(n_k):
                ...
                q_kite_si = V_init_si['x', ndx, q_kite_name]
                diff_si = V_init_si['z', ndx, var_name]
                node_position = vortex_tools.combine_diff_and_qkite_into_wake_node_position_si(q_kite_si, diff_si)
            ...
            for ndx in range(n_k):
                for ddx in range(d):
                    ...
                    q_kite_si = V_init_si['coll_var', ndx, ddx, 'x', q_kite_name]
                    diff_si = V_init_si['coll_var', ndx, ddx, 'z', var_name]
                    node_position = vortex_tools.combine_diff_and_qkite_into_wake_node_position_si(q_kite_si, diff_si)
...
def get_specific_local_initialization(abbreviated_var_name, init_options, V_init_scaled, Outputs, Integral_outputs_scaled, model, time_grids, kite_shed_or_parent_shed, tip,
    ...
    if abbreviated_var_name == 'wx':
        var_val_si = diff_fixing.get_local_convected_position_diff_value(init_options, V_init_scaled, Outputs, model, time_grids, kite_shed_or_parent_shed, tip, wake_node_or_ring, ndx_find, ddx_find)
    elif abbreviated_var_name == 'wg':
        var_val_si = diff_fixing.get_local_average_circulation_value(init_options, V_init_scaled, Integral_outputs_scaled, model, time_grids, kite_shed_or_parent_shed, wake_node_or_ring, ndx_find, ddx_find)
    elif abbreviated_var_name == 'wh':
        var_val_si = diff_fixing.get_local_cylinder_pitch_value(init_options, Outputs, kite_shed_or_parent_shed, wake_node_or_ring, ndx_find, ddx_find)
    elif abbreviated_var_name == 'wx_center':
        var_val_si = diff_fixing.get_local_cylinder_center_value(init_options, Outputs, kite_shed_or_parent_shed, wake_node_or_ring, ndx_find, ddx_find)
    else:
